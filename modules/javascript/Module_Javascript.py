...
class JS_Mod:
    ...
    def locateJavascriptSource(self):
        ...
        if self.mode == 0:
            ...
            binaryClassID = b'\xAE\xFD\x24\xAE\xC6\x03\xD1\x11\x8B\x76\x00\x80\xC7\x44\xF3\x89'
            ...
            if self.docType == '/word':
                ...
                if b'CONTROL ScriptBridge.ScriptBridge' in wordDocBuffer:
                    ...
                    for content in listOCXContents:
                        ...
                        if contentBuffer[0:16] == binaryClassID:
                            ...
                            for character in range(22, 22+(pathLength*2), 2):
                                pathToSourceFile = pathToSourceFile + chr(contentBuffer[character])
                            if not self.args.quiet and not self.args.json:
